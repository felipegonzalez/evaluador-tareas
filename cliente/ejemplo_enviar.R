...
enviar.fun <- function(nombre, my_key, funcion.archivo, tarea, ejercicio, nombre.fun){
   ...
   close(con)
   #print(texto.fun)
   html <- postForm(servidor.1, nombre = nombre, my_key=my_key, tarea=tarea,ejercicio=ejercicio, funcion=texto.fun, style="httppost",
      .encoding='utf-8')
   cat(rawToChar(html))
...
enviar <- function(){
   ...
   if(as.integer(parte)==1){
      enviar.fun(nombre=nombre, my_key=my_key, funcion.archivo='post_binomial.R', tarea="1",ejercicio="1", nombre.fun='ejemplo')
   }
